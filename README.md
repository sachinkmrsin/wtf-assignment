...
│   │   │   ├── migrations/     # 001_initial.sql (schema + core indexes + mat. view)
 │   │   │   │                     002_indexes.sql (supplementary covering/FK indexes)
...
Indexes are split across two migration files:

- **`001_initial.sql`** — core indexes created alongside the schema (required for correctness and the hottest queries)
- **`002_indexes.sql`** — supplementary indexes added after profiling; each is justified by a concrete query or FK constraint

#### Core Indexes (`001_initial.sql`)
...
| `idx_checkins_member`         | **Composite B-Tree** on `checkins(member_id, checked_in DESC)`                       | Supports member-level check-in history queries. Leading `member_id` equality filter, `checked_in DESC` satisfies ORDER BY without a sort step.                                                          |
...
| `idx_members_gym_id`          | **B-Tree** on `members(gym_id)`                                                      | Supports all gym-level member aggregations (`active_members`, `total_members`). Foreign-key enforcement on gym deletion.                                                                                |
...
#### Supplementary Indexes (`002_indexes.sql`)

| Index                      | Type                                                                                             | Reasoning                                                                                                                                                                                                                                 |
| -------------------------- | ------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `idx_checkins_gym_id`      | **B-Tree** on `checkins(gym_id)`                                                                 | The partial `idx_checkins_live_occupancy` only covers open check-ins. This plain index covers the full range needed for `today_checkins` and `weekly_checkins` aggregations used on every gym card.                                       |
| `idx_payments_date`        | **Covering B-Tree** on `payments(paid_at DESC, gym_id)` INCLUDE `(amount)` WITH `fillfactor=100` | Q5 groups by `gym_id` and sums `amount` over a date range. The covering index avoids heap fetches entirely — the planner satisfies the entire query from the index leaf pages. `fillfactor=100` is safe because payments are append-only. |
| `idx_payments_member_id`   | **B-Tree** on `payments(member_id)`                                                              | Supports ON DELETE CASCADE FK enforcement when a member is removed. Also a natural foundation for a future member-level payment history endpoint.                                                                                         |
| `idx_anomalies_gym_id`     | **B-Tree** on `anomalies(gym_id)`                                                                | Covers `resolved=TRUE` lookups not served by the partial `idx_anomalies_active`, plus FK cascade from gyms.                                                                                                                               |
| `idx_members_email_unique` | **Unique Partial B-Tree** on `members(email)` WHERE `email IS NOT NULL`                          | Data-integrity constraint preventing duplicate emails while allowing multiple `NULL` values. Also makes future email-based lookups (login, contact dedup) instant.                                                                        |
...
| #   | Query Name                   | Index Used                                | Target  | Screenshot                                |
| --- | ---------------------------- | ----------------------------------------- | ------- | ----------------------------------------- |
| Q1  | Live Occupancy — Single Gym  | `idx_checkins_live_occupancy` (partial)   | < 0.5ms | [Q1.png](./benchmarks/screenshots/Q1.png) |
| Q2  | Today's Revenue — Single Gym | `idx_payments_gym_date` (composite)       | < 0.8ms | [Q2.png](./benchmarks/screenshots/Q2.png) |
| Q3  | Churn Risk Members           | `idx_members_churn_risk` (partial)        | < 1ms   | [Q3.png](./benchmarks/screenshots/Q3.png) |
| Q4  | Peak Hour Heatmap (7d)       | `idx_gym_hourly_stats_unique` (mat. view) | < 0.3ms | [Q4.png](./benchmarks/screenshots/Q4.png) |
| Q5  | Cross-Gym Revenue Comparison | `idx_payments_date` (covering)            | < 2ms   | [Q5.png](./benchmarks/screenshots/Q5.png) |
| Q6  | Active Anomalies — All Gyms  | `idx_anomalies_active` (partial)          | < 0.3ms | [Q6.png](./benchmarks/screenshots/Q6.png) |

> `EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT)` screenshots captured against the seeded dataset — see [`benchmarks/README_BENCHMARKS.md`](./benchmarks/README_BENCHMARKS.md) for the exact SQL used.
...
- **Frontend design no upto standards** — the UI is functional but not pixel-perfect. A production system would invest more in design polish and responsiveness.
