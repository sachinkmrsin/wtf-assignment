...
jest.mock('../../src/services/statsService', () => ({
  ...
  getGymStats:  jest.fn().mockResolvedValue({
    gymId: 'gym-uuid-1', todayRevenue: 500, weeklyCheckins: 120,
  }),
}));
...
describe('simulateCheckin', () => {
  it('inserts a check-in record and broadcasts gym:checkin + gym:occupancy events', async () => {
    mockQuery
      .mockResolvedValueOnce({ rows: [{ id: 'member-uuid-1', name: 'Alice', plan_type: 'monthly' }] }) // getRandomMemberForGym
    ...
    expect(mockBcastGym).toHaveBeenCalledWith(
      'gym-uuid-1',
      'gym:checkin',
      expect.objectContaining({ gymId: 'gym-uuid-1', memberId: 'member-uuid-1', memberName: 'Alice', currentOccupancy: 5, capacityPct: 5 }),
    );
    expect(mockBcastGym).toHaveBeenCalledWith(
      'gym-uuid-1',
      'gym:occupancy',
...
describe('simulateCheckout', () => {
  it('closes the open check-in and broadcasts a gym:checkout event', async () => {
    mockQuery
      .mockResolvedValueOnce({ rows: [{ id: 'checkin-uuid-1', member_id: 'member-uuid-1', member_name: 'Bob' }] }) // SELECT with JOIN
    ...
    expect(mockBcastGym).toHaveBeenCalledWith(
      'gym-uuid-1',
      'gym:checkout',
      expect.objectContaining({ gymId: 'gym-uuid-1', checkinId: 'checkin-uuid-1', memberName: 'Bob', currentOccupancy: 4 }),
    );
  });
...
describe('simulatePayment', () => {
  it('records a payment and broadcasts a payment:new event with a realistic amount', async () => {
    mockQuery
      .mockResolvedValueOnce({ rows: [{ id: 'member-uuid-1', name: 'Carol', plan_type: 'quarterly' }] }) // random member
    ...
    expect(mockBcastGym).toHaveBeenCalledWith(
      'gym-uuid-1',
      'payment:new',
      expect.objectContaining({ gymId: 'gym-uuid-1', memberId: 'member-uuid-1', memberName: 'Carol', planType: 'quarterly', todayTotal: 520 }),
    );
...
describe('runSimulatorTick', () => {
  ...
  it('executes a check-in when the random action value is below 0.45 (45% probability)', async () => {
    ...
    expect(mockBcastGym).toHaveBeenCalledWith(
      'gym-uuid-1', 'gym:checkin', expect.any(Object),
    );
    spy.mockRestore();
  });
  ...
  it('executes a checkout when the random action value is between 0.45 and 0.75 (30% probability)', async () => {
    ...
    mockQuery
      .mockResolvedValueOnce({ rows: [{ id: 'checkin-uuid-1', member_id: 'member-uuid-1', member_name: 'Eve' }] })
      .mockResolvedValueOnce({ rows: [] })
    ...
    expect(mockBcastGym).toHaveBeenCalledWith(
      'gym-uuid-1', 'gym:checkout', expect.any(Object),
    );
    spy.mockRestore();
  });
});
